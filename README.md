...
**NVIDIA only** — requires `nvidia-smi`
...
## Requirements
- NVIDIA GPU only
- `nvidia-smi` must be installed (comes with NVIDIA drivers)
...
## 需求
- 只支援 NVIDIA 顯示卡
- 必須安裝 `nvidia-smi`（隨 NVIDIA 驅動程式附帶）
...
## 必要環境
- NVIDIA GPU のみ対応
- `nvidia-smi` インストール済み（NVIDIA ドライバーに付属）
